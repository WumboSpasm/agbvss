...
void InitScroll()
{
	...
	DmaArrayCopy(3,Lev1bg2_Character,CHAR_BASE1_ADDR,16);	// Set tile data for bg1 middle layer.
	InitMiddle();
	DmaArrayCopy(3,Bg2_ScreenDat,CHAR_BASE3_ADDR+0x2800,16);

	DmaArrayCopy(3,Lev1bg1_Palette,BG_PLTT,16);				// Set tile data for bg2 action layer.
...
// Initialise the 'bg2' middle layer map.
...
void InitMiddle()
{
	...
	for (y=0;y<32;y++)
	{
		for (x=0;x<32;x++)
		{
			Bg2_ScreenDat[y*32+x]=Lev1bg2_Map[(y*256>>3)+x]; // Update screen map buffer with tile name data.
		}
	}
}
...
// Do the DMA copy and fine scroll updates first.
	...
	*(vu16*)REG_BG1HOFS=map_xpos>>1;
	*(vu16*)REG_BG1VOFS=map_ypos>>2;
	*(vu16*)REG_BG2HOFS=x_shift;
	*(vu16*)REG_BG2VOFS=y_shift;
	*(vu16*)REG_BG3HOFS=map_xpos<<1;
	*(vu16*)REG_BG3VOFS=0;			// Currently fixed value !.

//--

	x_tilepos=map_xpos>>3;	// Get map x coord and divide by '8' to get 8*8 tile rez and store it.
	x_shift=map_xpos&0x0007;// Isolate amount of x fine shift and store it.

	y_tilepos=map_ypos>>3;	// Get map y coord and divide by '8' to get 8*8 tile rez and store it.
	y_shift=map_ypos&0x0007;// Isolate amount of y fine shift and store it.

	for (y=0;y<21;y++) 	// Y size of screen buffer in 8x8 tiles.
	{
		for (x=0;x<31;x++) // X size of screen buffer in 8x8 tiles.
		{
			Bg1_ScreenDat[y*32+x]=Lev1bg1_Map[((y_tilepos+y)*map_x_size_tiles)+x_tilepos+x]; // Update screen map buffer with tile name data.
		}
	}
